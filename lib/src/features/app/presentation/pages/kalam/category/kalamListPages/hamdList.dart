...
class _HamdListState extends State<HamdList> {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return GestureDetector(
                    ...
                    onLongPress: () {
                      _showPopupMenu(context, hamdlist[index]);
                    },
                    child: Container(
  ...
  void _showPopupMenu(BuildContext context, Hamd hamd) {
    final size = MediaQuery.of(context).size;
  ...
  Future<void> _downloadHamd(Hamd hamd) async {
    // Request storage permissions
    if (await Permission.storage.request().isGranted) {
      // Get the directory to save the file
      final directory = await getExternalStorageDirectory();
      final file = File('${directory!.path}/${hamd.lines.first}.txt');

      // Write the Hamd text to the file
      await file.writeAsString(hamd.lines.join('\n'));

      // Show a confirmation message
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${hamd.lines.first} has been downloaded'),
        ),
      );
    } else {
